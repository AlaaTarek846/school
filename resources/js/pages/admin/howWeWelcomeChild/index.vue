...
const getData = (page = 1) => {
    loading.value = true;
    adminApi.get(`how-we-welcome-child?page=${page}`)
        .then((res) => {
...
const openModal = () => {
    type.value = 'create';
    dataRow.value = {};
    let myModal = new bootstrap.Modal(document.getElementById('how-we-welcome-child-model'), {
        keyboard: false
    });
    myModal.show();
}
...
const deleteItem = (id) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            adminApi.delete(`how-we-welcome-child/${id}`)
                .then((res) => {
                    Swal.fire(
                        t('global.Deleted'),
