  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row">
          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('global.name') }} <span class="text-danger">*</span></label>
            <input type="text" class="form-control" v-model="v$.name.$model" :class="{'is-invalid': v$.name.$error || errors['name']}">
          ...
          <div class="col-md-6 mb-2">
            ...
            <input type="email" class="form-control" v-model="v$.email.$model" :class="{'is-invalid': v$.email.$error || errors['email']}">
            <div class="invalid-feedback">
          ...
          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('admin.code') }} <span class="text-danger">*</span></label>
            <input type="text" class="form-control" v-model="v$.code.$model" :class="{'is-invalid': v$.code.$error || errors['code']}">
            <div class="invalid-feedback">
          ...
          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('admin.username') }} <span class="text-danger">*</span></label>
            <input type="text" class="form-control" v-model="v$.username.$model" :class="{'is-invalid': v$.username.$error || errors['username']}">
            <div class="invalid-feedback">
          ...
          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('global.password') }} <span v-if="type == 'create'" class="text-danger">*</span></label>
            <input type="password" class="form-control" v-model="v$.password.$model" :class="{'is-invalid': v$.password.$error || errors['password']}">
            <div class="invalid-feedback">
          ...
          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('admin.gender') }} <span class="text-danger">*</span></label>
            <select class="form-control" v-model="v$.gender.$model" :class="{'is-invalid': v$.gender.$error || errors['gender']}">
              <option value="male">{{ $t('admin.male') }}</option>
              <option value="female">{{ $t('admin.female') }}</option>
            </select>
            <div class="invalid-feedback">
              <span v-if="v$.gender.required.$invalid">{{ $t('validation.fieldRequired') }}</span>
            </div>
          </div>

          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('admin.birth_day') }}</label>
            <input type="date" class="form-control" v-model="submitData.data.birth_day" :class="{'is-invalid': errors['birth_day']}">
          </div>

          <div class="col-md-6 mb-2">
             <!-- Empty space or can move phone up -->
          </div>

          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('admin.phone_1') }}</label>
            <input type="text" class="form-control" v-model="v$.phone_1.$model" :class="{'is-invalid': v$.phone_1.$error || errors['phone_1']}">
          </div>

          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('admin.phone_2') }}</label>
            <input type="text" class="form-control" v-model="v$.phone_2.$model" :class="{'is-invalid': v$.phone_2.$error || errors['phone_2']}">
          </div>

          <div class="col-md-4 mb-2">
            <label class="form-label">{{ $t('admin.governorate') }}</label>
            <input type="text" class="form-control" v-model="submitData.data.governorate" :class="{'is-invalid': errors['governorate']}">
          </div>

          <div class="col-md-4 mb-2">
            <label class="form-label">{{ $t('admin.city') }}</label>
            <input type="text" class="form-control" v-model="submitData.data.city" :class="{'is-invalid': errors['city']}">
          </div>

          <div class="col-md-4 mb-2">
            <label class="form-label">{{ $t('admin.address') }}</label>
            <input type="text" class="form-control" v-model="submitData.data.address" :class="{'is-invalid': errors['address']}">
          </div>

          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('admin.academic_year') }} <span class="text-danger">*</span></label>
            <select class="form-control" v-model="v$.academic_year_id.$model" :class="{'is-invalid': v$.academic_year_id.$error || errors['academic_year_id']}">
              <option v-for="year in academicYears" :key="year.id" :value="year.id">{{ year.name }}</option>
            </select>
          </div>

          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('admin.semester') }} <span class="text-danger">*</span></label>
            <select class="form-control" v-model="v$.semester_id.$model" :class="{'is-invalid': v$.semester_id.$error || errors['semester_id']}">
          ...
          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('admin.education_stage') }} <span class="text-danger">*</span></label>
            <select class="form-control" v-model="v$.education_stage_id.$model" :class="{'is-invalid': v$.education_stage_id.$error || errors['education_stage_id']}">
              <option v-for="stage in educationStages" :key="stage.id" :value="stage.id">{{ $i18n.locale == 'ar' ? stage.title_ar : stage.title_en }}</option>
            </select>
          </div>

          <div class="col-md-6 mb-2">
            <label class="form-label">{{ $t('admin.school_class') }} <span class="text-danger">*</span></label>
            <select class="form-control" v-model="v$.school_class_id.$model" :class="{'is-invalid': v$.school_class_id.$error || errors['school_class_id']}">
          ...
          <div class="col-md-6 mt-2">
            <div class="custom-toggle-switch d-flex align-items-center mt-4">
              <input id="is_activeToggle" v-model="submitData.data.is_active" type="checkbox">
              <label for="is_activeToggle" class="label-primary"></label><span class="ms-3">{{ $t('admin.status') }}</span>
            </div>
          </div>
          
          <div class="col-md-6 mt-2">
            <div class="custom-toggle-switch d-flex align-items-center mt-4">
              <input id="is_completedToggle" v-model="submitData.data.is_completed" type="checkbox">
              <label for="is_completedToggle" class="label-primary"></label><span class="ms-3">{{ $t('admin.is_completed') }}</span>
            </div>
          </div>

          <div class="col-md-12 mt-2" v-if="Object.keys(errors).length > 0">
            <error-message v-for="(errMsg, errKey) in errors" :key="errKey">
              {{ errMsg[0] }}
            </error-message>
          </div>
      ...
      <div class="modal-footer">
        <button v-if="type != 'edit'" :disabled="!is_disabled" @click.prevent="resetModal" type="button" class="btn btn-secondary">{{$t('global.AddNewRecord')}}</button>
...
function defaultData() {
  ...
  submitData.data.academic_year_id = academicYears.value.length ? academicYears.value[0].id : '';
