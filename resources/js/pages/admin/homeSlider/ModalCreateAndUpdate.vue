                ...
                <div class="modal-body">
                    <div class="row">
                      ...
                      <div class="col-md-12 mt-3">
                          <label class="form-label">صورة  (900 * 1920)</label>
...
<script setup>
  ...
  import {maxLength, minLength, required, numeric, requiredIf, integer,url} from "@vuelidate/validators";
  ...
  function resetModal() {
    ...
    setTimeout(async () => {
      if (props.type != 'edit') {
        ...
        adminApi.get(`home-sliders/${id.value}`)
            .then((res) => {
              loading.value = true;
              let l = res.data.data;
  ...
  const AddSubmit = () =>  {
      ...
      if (props.type !== 'edit') {
        if (!v$.value.$error) {
          is_disabled.value = false;
          loading.value = true;
          adminApi.post(`home-sliders`, formData)
              .then((res) => {
                Swal.fire({
                  icon: 'success',
        ...
        adminApi.post(`home-sliders/${id.value}`,formData)
            .then((res) => {
              Swal.fire({
                icon: 'success',
