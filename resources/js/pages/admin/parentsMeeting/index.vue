                    ...
                    <div class="table-responsive">
                        <table class="table text-nowrap table-bordered">
                            ...
                            <tr>
                                ...
                                <th scope="col">تفاصيل الاجتماع</th>
                            ...
                            <tr v-for="(item, index) in records" :key="item.id">
                                ...
                                <td>{{ item.title_en }}</td>
                                <td>
                                    <div v-if="item.is_general_time" class="mb-1 text-muted small">
                                        وقت عام: {{ formatTimeRange(item.time_from, item.time_to) }}
                                    </div>
        ...
        async getResults(page = 1) {
            try {
                const response = await adminApi.get(
                    `/parents-meetings?page=${page}&search=${this.search}`
                );
                this.records = response.data.data;
                this.pagination = response.data.pagination;
            } catch (error) {
                console.error("Error fetching parents meetings:", error);
            }
        },
        openModal() {
        ...
        async deleteItem(id) {
            try {
                ...
                if (result.isConfirmed) {
                    await adminApi.delete(`/parents-meetings/${id}`);
                    this.getResults();
                    Swal.fire(
